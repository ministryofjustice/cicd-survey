...
output_file="circleci_authors.csv"
...
while IFS= read -r repo_name; do
  if [ -z "$repo_name" ]; then
    ...
    if [ -d "$repo_dir" ]; then
      ...
      # Get the commit history for the past year for .circleci directory
      git log --since="1 year ago" --pretty=format:"%ae" --numstat -- .circleci > /tmp/gitlog.txt
